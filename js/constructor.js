...
$(function () {
  ...
  for (let i = 0; i < controls.length; i++) {
    let group = controls[i];
    for (let key in group) {
      let options = group[key].data;
      let placeholder = group[key].placeholder;
      let title = group[key].title;
  ...
  $constructor.find('select').change(function () {
    ...
    switch (name) {
      case 'tumba':
        ...
        break;
      case 'tzvet':
        $('[name="nadgrob"]').val('d00');
        $('#nadgrob').find('img').attr('src', 'images/d00.png');
        break;
      case 'nadgrob':
        $('[name="tzvet"]').val('c00');
        $('#tzvet').find('img').attr('src', 'images/c00.png');
        break;
      case 'risynki':
      ...
      case 'krest':
        ...
        break;
      case 'epitaph':
        value = value !== 'e00' ? 'ep/' + value + vl_stella + vl_port : 'ep/' + value;
        break;
    ...
    $('#' + name).find('img').attr('src', 'images/' + value + '.png');
  });
